...
** NOTE: it's a good time to remove the mapper dependencies from SetMessagesUpdateProcessor
** NOTE: and also port SetMessagesMethodTest to cucumber
...
** NOTE: we have to be carefull on the message id we are returning (old MailboxPath, not the new MailboxPath)
